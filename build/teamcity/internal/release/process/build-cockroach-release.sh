...
if [[ -z "${DRY_RUN}" ]] ; then
  gcs_bucket="cockroach-release-artifacts-staging-prod"
  ...
  if [[ -z "${PRE_RELEASE}" ]] ; then
    ...
    gcr_repository="us-docker.pkg.dev/cockroach-cloud-images/cockroachdb/cockroach-unstable"
  fi
  # Used for docker login for gcloud
  gcr_hostname="us-docker.pkg.dev"
  git_repo_for_tag="cockroachdb/cockroach"
else
  gcs_bucket="cockroach-release-artifacts-staging-dryrun"
