...
if [[ -z "${DRY_RUN}" ]] ; then
  ...
  s3_download_hostname="${s3_bucket}"
  ...
  s3_download_hostname="${s3_bucket}.s3.amazonaws.com"
...
if [[ -n "${PUBLISH_LATEST}" && -z "${PRE_RELEASE}" ]]; then
    BAZEL_SUPPORT_EXTRA_DOCKER_ARGS="-e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e TC_BUILDTYPE_ID -e TC_BUILD_BRANCH=$build_name -e s3_bucket -e gcs_credentials -e gcs_bucket" run_bazel << 'EOF'
bazel build --config ci //pkg/cmd/publish-provisional-artifacts
BAZEL_BIN=$(bazel info bazel-bin --config ci)
