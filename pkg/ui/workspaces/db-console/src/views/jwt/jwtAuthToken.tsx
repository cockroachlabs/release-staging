...
import { Button, CockroachLabsLockupIcon, Text, TextInput, TextTypes } from "src/components";
...
export const JwtAuthToken = (props: { loading: boolean, error: string, info: ConnectionInfo }) => {
  const renderLoading = () => {
    return <Spinner />
  }
  ...
  const renderInfo = (info: ConnectionInfo) => {
    const [username, setUsername] = useState(info.Usernames[0]);

    const usernameOptions = info.Usernames.map((u) => { return { label: u, value: u } });

    return (
      <div>
        <p>Use the following values to configure your client connection to the cluster.</p>
        <div className="crl-input__wrapper">
          <label htmlFor="username" className="crl-input__label">Username</label>
          <Select
            name="username"
            clearable={false}
            value={username}
        ...
        <TextInput label={`Password (Expires ${info.Expiry}.)`} value={info.Password} initialValue={info.Password} onChange={() => {}}/>
        <TextInput label="Host" value={info.Host} initialValue={info.Host} onChange={() => {}}/>
        <TextInput label="Port" value={info.Port.toString()} initialValue={info.Port.toString()} onChange={() => {}}/>
        <TextInput label="Options" value={"--crdb:jwt_auth_enabled=true"} initialValue={"--crdb:jwt_auth_enabled=true"} onChange={() => {}}/>
        {renderButton()}
      </div>
    )
  }
    ...
    <div className="jwt-auth-token">
      ...
      <div className="jwt-auth-token__container">
        ...
        <div className="content">
          ...
          <section className="jwt-auth-token__documentation">
          </section>
        </div>
      </div>
    </div>
...
export const JwtAuthTokenPage = () => {
  ...
  useEffect(() => {
    // TODO(todd): try/catch here?
    const { State, Code } = JSON.parse(atob(oidc));

    // TODO(todd): Custom marshaling for lowercase field names?
    fetch(`/oidc/v1/jwt?state=${State}&code=${Code}`).then((response: Response) => {
